...
export const ReportCard: React.FC<ReportCardProps> = ({ report, onAnalyse, onDelete }) => {
  ...
  useEffect(() => {
    // Automatically open the card when analysis starts or completes.
  ...
  const cardRingColor = useMemo(() => {
    if (report.status === 'failed') return 'ring-gray-400';
    if (hasAbnormalResults) return 'ring-red-300';
    ...
    <div className="text-center py-6">
        ...
        <div className="flex justify-center items-center gap-4">
            <button
                ...
                className="bg-gray-200 text-dark font-bold py-2 px-4 rounded-lg hover:bg-gray-300 transition-colors duration-200"
            >
                View Document
            </button>
            <button
                onClick={() => onAnalyse(report.id)}
                className="bg-primary text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-800 transition-colors duration-200"
            >
                Analyse Report
            </button>
        </div>
  ...
  return (
    <div className={`bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 ring-2 ${cardRingColor}`}>
      <div className="w-full flex justify-between items-center p-4 md:p-6 text-left">
        ...
        >
          <div>
            ...
            <h3 className="text-lg font-semibold text-primary break-all">{report.fileName}</h3>
            <div className="mt-1">
                ...
                {hasAbnormalResults && <span className="px-3 py-1 text-xs font-bold text-white bg-red-500 rounded-full">Action Required</span>}
